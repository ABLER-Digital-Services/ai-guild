...
以下の `fixtures/`、`base.ts`、`test-data.ts` などの名称や配置は説明用の例です。実際のディレクトリ構成は対象リポジトリ側の E2E 規約に従ってください。
...
precondition カタログに、使用可能な precondition を定義する。spec の YAML ではこのカタログに存在する key のみを `precondition` に使える。
...
対象リポジトリ側の fixture 定義ファイルに fixture を追加する。
...
precondition カタログに新しいエントリを追加する。
...
- [ ] 対象リポジトリ側の precondition カタログに登録したか
- [ ] fixture の依存先が存在するか
